...
n_indicators = 1
refrence = 1           # refrence cut for selecting max value (l)
...
def plot_viol_prob_cl_cost():
    ...
    ax.hist(violation_prob_cl_cost,density=True, bins=5)
    ...
    ax.legend()
    plt.savefig('violation_prob_ms_cl_cost_mshaped_high1_0.1.pdf')
...
def plot_viol_prob_batt_constr():
    ...
    fig, ax = plt.subplots(1,1)
    ax.hist(violation_prob_batt_constr,density=True, bins=4)
...
def plot_viol_prob_batt_constr_max():
        ...
        fig, ax = plt.subplots(1,1)
        ax.hist(violation_prob_batt_constr_max,density=True, bins=4)
        ...
        ax.plot(epsilon,result, color='red', label='PDF defined by the scenario approach ($f_{SA}(ϵ)$)')
        ax.legend()
        plt.savefig('violation_prob_ms_batt_constr_max_mshaped_high1_0.1.pdf')
...
def plot_viol_prob_dual_2():
    ...
    ax.plot(epsilon,result, color='red', label='PDF defined by the scenario approach ($f_{SA}(ϵ)$)')
    ax.legend()
    plt.savefig('violation_prob_ms_dual_2_mshaped_high1_0.1.pdf')
