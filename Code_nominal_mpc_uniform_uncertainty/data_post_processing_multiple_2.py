...
def plot_viol_prob_cl_cost():
    ...
    ax.legend()
    #plt.savefig('violation_prob_cl_cost_uniform_0.1.pdf')
...
def plot_viol_prob_batt_constr_max():
        ...
        ax.plot(epsilon,result, color='red', label='PDF defined by the scenario approach ($f_{SA}(ϵ)$)')
        ax.legend()
        plt.savefig('violation_prob_batt_constr_max_uniform_0.1.pdf')
...
def plot_viol_prob_dual_2():
    ...
    ax.plot(epsilon,result, color='red', label='PDF defined by the scenario approach ($f_{SA}(ϵ)$)')
    ax.legend()
    #plt.savefig('violation_prob_dual_2_uniform_0.1.pdf')
