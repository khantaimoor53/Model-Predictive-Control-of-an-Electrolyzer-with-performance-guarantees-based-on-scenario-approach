...
res_global = res_global[0:7000] # select the no. of samples to be processed
n_samples = 1000 # cut size
...
n_indicators = 1
refrence = 2           # refrence cut for selecting max value (l)
...
def plot_viol_prob_cl_cost():
    ...
    ax.hist(violation_prob_cl_cost,density=True, bins=3)
    ...
    ax.axvline(x = 0.01, color='black', linestyle='--', label='Violation probability bound ($ϵ$)')
...
def plot_viol_prob_batt_constr():
    ...
    ax.axvline(x=(n_indicators/(n_samples+1)), color='orange', linestyle='--', label='Mean calculated violation probability')
    ax.axvline(x = 0.01, color='black', linestyle='--', label='Violation probability threshold ( $\epsilon$ )')
...
def plot_viol_prob_batt_constr_max():
        ...
        fig, ax = plt.subplots(1,1)
        ax.hist(violation_prob_batt_constr_max,density=True, bins=2)
        ...
        ax.axvline(x=(n_indicators/(n_samples+1)), color='orange', linestyle='--', label='Mean calculated violation probability')
        ax.axvline(x = 0.01, color='black', linestyle='--', label='Violation probability threshold (ϵ)')
        ax.set_xlabel('Violation probability ($V_p$))')
        ax.set_ylabel('Probability density')
        ax.set_xlim(0,0.015)
        ...
        ax.plot(epsilon,result, color='red', label='PDF defined by the scenario approach ($f_{SA}(ϵ)$)')
        ax.legend()
        plt.savefig('violation_prob_batt_constr_max_uniform_0.01.pdf')
...
def plot_viol_prob_dual():
    ...
    for i in np.arange(0,1,0.0001):
        ...
        result.append(1000*999*i*(1-i)**998)
        epsilon.append(i)
    ax.plot(epsilon,result, color='red', label='SP')
    ax.legend()
...
def plot_viol_prob_dual_2():
    ...
    fig, ax = plt.subplots(1,1)
    ax.hist(violation_prob_dual,density=True, bins=4)
    ax.axvline(x=(sum(violation_prob_dual)/len(violation_prob_dual)), color='g', linestyle='--', label='Mean observed violation probability')
    ax.axvline(x=(2/(n_samples+1)), color='orange', linestyle='--', label='Mean calculated violation probability')
    ax.axvline(x = 0.01, color='black', linestyle='--', label='Violation probability bound ($ϵ$)')
    ax.set_xlabel('Violation probability ($V_p$)')
    ...
    ax.plot(epsilon,result, color='red', label='PDF defined by the scenario approach ($f_{SA}(ϵ)$)')
    ax.legend()
    plt.savefig('violation_prob_dual_2_uniform_0.01.pdf')
